...
- `/admin/<id>`: зүүн талд гарчиг, хураангуй, үндсэн текст, шошго, slug-ийн форм
  ...
  хуудсан дээр харагдана), Facebook төлөв ба «Одоо FB-д постлох».
...
```bash
npm run publish:facebook    # дараалалаас FB_POSTS_PER_RUN ширхгийг постолно
```
...
**Дараалал.** `status = PUBLISHED` бөгөөд `fbPostedAt` хоосон нийтлэлүүд дараалалд байна.
Эрэмбэ: хамгийн сүүлд нийтлэгдсэн, тэнцвэл оноо өндөр нь. Амжилттай бол `fbPostId`, `fbPostedAt`
бичигдэнэ. Алдаа гарвал `fbPostedAt` хоосон үлдэж **дараагийн run-д дахин оролдоно**; `fbAttempts`
нэмэгдэж, `fbError`-т сүүлийн алдаа бичигдэнэ. 3 удаа унасан нийтлэл дараалалаас гарна
(/admin дээр улаанаар «3 удаа алдаа» гэж харагдана).
...
2–3 өгүүлбэр lead (хураангуй, дутвал биетийн эхнээс нөхнө)
...
(`/api/og/<slug>`). Hashtag: `#AI #ХиймэлОюун` + холбогдсон модель/компанийн нэрнээс нэг.

`/admin`-ы PUBLISHED таб болон `/admin/<id>` дээр FB төлөв (постлосон огноо эсвэл «хүлээгдэж байна»)
харагдаж, «Одоо FB-д постлох» товчоор дараалал хүлээхгүйгээр гараар постолж болно.
...
3. **Ижил сэдэв/модель давхардуулахгүй** — ижил модель дурдсан, эсвэл ижил компани + ижил
   шошготой мэдээнээс нэгийг л авна.
4. Зөвхөн **бүтэн тексттэй** (`sourceText`) нийтлэл — хураангуйгаар бичигдсэнд баримт дутуу байж мэднэ.
